...
namespace CosmoPlus
{
    public partial class MalzemeTipiEkleForm : Form
    {
        string connectionString = System.Configuration.ConfigurationManager.ConnectionStrings["MyConnectionString"].ConnectionString;

        public MalzemeTipiEkleForm()
        {
            InitializeComponent();
            LoadFirmaCombobox();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
         ...
         //   string KodQuery = "SELECT COMCODE FROM BSMGR0GEN001 WHERE UPPER(COMTEXT) = UPPER(@COMTEXT)";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                  ...
                  //  COMCODE = ulkeKoduCommand.ExecuteScalar()?.ToString();
                    ...
                    string insertQuery = "INSERT INTO BSMGR0MAT001 ( DOCTYPE, DOCTYPETEXT , ISPASSIVE ) " +
                                         "VALUES (@DOCTYPE, @DOCTYPETEXT , @ISPASSIVE)";
                  ...
                  //  insertCommand.Parameters.Add("@COMCODE", SqlDbType.NVarChar).Value = COMCODE;
                    ...
                    if (rowsAffected > 0)
                    {
                        MessageBox.Show(" Melzeme Tipi başarıyla eklendi!", "Bilgi", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        MessageBox.Show("Melzeme Merkezi eklenemedi!", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                catch (Exception ex)
                {
            ...
            ViewData vd = new ViewData("BSMGR0MAT001");
            vd.Show();
            this.Hide();
        }
